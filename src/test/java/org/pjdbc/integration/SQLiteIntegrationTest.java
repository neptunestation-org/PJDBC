...
public class SQLiteIntegrationTest {
    ...
    void logDriverLogsQueries() throws SQLException {
        String url = "jdbc:log:" + SQLITE_FILE_URL;
        try (Connection conn = DriverManager.getConnection(url)) {
            try (Statement stmt = conn.createStatement();
    ...
    void poolDriverMaintainsConnections() throws SQLException {
        String url = "jdbc:pool[min=1,max=3]:" + SQLITE_FILE_URL;
    ...
    void hikariPoolDriverWorks() throws SQLException {
        String url = "jdbc:hikaricp:" + SQLITE_FILE_URL + "?maximumPoolSize=2";
        try (Connection conn = DriverManager.getConnection(url)) {
            assertNotNull(conn);
            try (Statement stmt = conn.createStatement();
    ...
    void cachingDriverCachesResults() throws SQLException {
        String url = "jdbc:cache[ttl=60]:" + SQLITE_FILE_URL;
        try (Connection conn = DriverManager.getConnection(url)) {
    ...
    void metricsDriverCollectsStats() throws SQLException {
        String url = "jdbc:metrics:" + SQLITE_FILE_URL;
        try (Connection conn = DriverManager.getConnection(url)) {
            try (Statement stmt = conn.createStatement()) {
    ...
    void tracingDriverCreatesSpans() throws SQLException {
        String url = "jdbc:trace:" + SQLITE_FILE_URL;
    ...
    void driverChainingWorks() throws SQLException {
        // Chain: log -> cache -> pool -> sqlite
        String url = "jdbc:log:jdbc:cache:jdbc:pool:" + SQLITE_FILE_URL;
        try (Connection conn = DriverManager.getConnection(url)) {
            try (Statement stmt = conn.createStatement();
